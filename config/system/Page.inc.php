...
class Page
{
    ...
    private static function render_template_alternate()
    {
        ...
        // Checking cache time
        if(dsCache::is_modified($file_gen_md5)){
    ...
    private static function slice_initialize($render_temp)
    {
        ...
        $slice_filter_pattern = '/\@import\(\'(.*)\'\,\'(.*)\'\)/iXsuUm';
        // get all import text
        ...
        for ($i=0; $i < $tab_next_slice; $i++) { 
            ...
            if($rgx_slice_count > 0){
                for ($j=0; $j < $rgx_slice_count; $j++) { 
                    ...
                    $rgx_source_compiled = [];
                    preg_match('/(?s)(?<=\@slice\(\''.$rgx_slice_match[1][$j].'\'\))(.*?)(?=\@endslice)/i',
